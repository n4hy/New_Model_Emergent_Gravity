...
\section{What is proved about torsion, and what is not}

In Einstein--Cartan, three statements are theorems of the field
equations \cite{Hehl1976,HehlDatta1971}:
\begin{enumerate}[leftmargin=1.6em]
\item the Cartan equation is algebraic;
\item torsion is locked pointwise to spin density;
\item torsion vanishes in vacuum and does not propagate.
\end{enumerate}
There is therefore no torsional wave, no torsional Hubble friction,
and no torsional substitute for $\Lambda$ or for cold dark matter
coming from the connection itself.

What is \emph{not} a theorem is ``torsion matters in spintronics and
nowhere in cosmology.'' That mixes three different objects.

\paragraph{Spacetime EC torsion at laboratory spin density.}
The Hehl--Datta contact is $\mathcal{L}_{\mathrm{HD}}\sim G\,J_5\cdot J_5$.
It is Planck-suppressed. The series already said it is invisible at
collider energies. It is equally invisible in a spintronic device:
ordered spin does not cancel the $1/M_{\mathrm{Pl}}^{2}$. Calling
spacetime EC torsion ``important in spintronics'' is \textbf{not
proved} and, as an EC spacetime effect, is almost certainly false.

\paragraph{Effective spin connections in materials.}
Condensed matter has Berry curvature, spin--orbit coupling, and
effective geometric connections for spin currents. Those can matter
in spintronics. They are \textbf{not} the $\omega$ of
\eqref{eq:pal}. Identifying them with Einstein--Cartan torsion is
an analogy, not a derivation.

\paragraph{Cosmology.}
For unpolarized FLRW matter the spin averages down and
$\kappa\times(\mathrm{spin\ density})$ is negligible compared with
$T_{\mu\nu}$ after (and, for ordinary estimates, during) standard
cosmological epochs \cite{Hehl1976}. That is a sourced estimate,
not a no-go for every extension. The only cosmological EC story in
the series is a Trautman--Pop\l awski bounce at nuclear or higher
aligned spin density, already tagged \pC{}, and that is not
late-universe cosmology. Propagating-torsion theories are a
different Lagrangian and are not this theory.

\paragraph{This program.}
Cosmology here is the metric sector. The axial obstruction of
Theorem~\ref{thm:obs} is not a cosmological crisis. It is a
mismatch between a nonlocal CFT current two-point function and an
algebraic contact. Do not promote that mismatch into a statement
about galaxies, and do not promote spintronics into a proof that
spacetime torsion is a laboratory field.
