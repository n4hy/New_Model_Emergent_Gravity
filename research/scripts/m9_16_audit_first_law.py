#!/usr/bin/env python3
"""First-law audit: dS vs Tr(K_CHM dC). No solver import.

1d N=160 L=20. 3d N=10 R=3, all interior NN. ε=0.015.

Writes ../data/m9_16_audit_first_law.json
"""
...
def audit_1d() -> dict:
    ...
    w = (ell / 2.0) ** 2 - x**2
    pairs = [(i, i + 1) for i in range(ell - 1)]
    ...
    for i, j in pairs:
        ...
        ds.append(s - s0)
        pred.append(float(np.trace(kchm @ (c - c0))))
    rho = pearson(ds, pred)
    return {"rho": rho, "R_shape": rshape(ds, pred), "C0": bool(rho > 0.80)}
...
def main() -> int:
    ...
    payload = {
        "task": "m9.16_audit_first_law",
        "method": "dS vs Tr(K_CHM dC); 1d N=160 L=20; 3d N=10 R=3 all NN",
        "one_d": d1,
        "three_d": d3,
        "verdicts": {"first_law": "CONFIRMED" if ok else "REFUTED"},
    }
    os.makedirs(DATA, exist_ok=True)
    path = os.path.join(DATA, "m9_16_audit_first_law.json")
