#!/usr/bin/env python3
"""Linear-functional audit. N=10, R=2, σ=1.5, ε=0.08. No import.

Writes ../data/m9_25_audit_linear.json
"""
...
CLIP = 1e-12
N, RADIUS, SIGMA, EPS = 10, 2, 1.5, 0.08
...
def main() -> int:
    ...
    for x in range(N):
        for y in range(N):
            for z in range(N):
                r2 = (x - cen) ** 2 + (y - cen) ** 2 + (z - cen) ** 2
                pot[ix(x, y, z)] = EPS * np.exp(-r2 / (2 * SIGMA * SIGMA))
    ham1 = ham.copy()
    ...
    path = os.path.join(DATA, "m9_25_audit_linear.json")
