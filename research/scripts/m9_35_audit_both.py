#!/usr/bin/env python3
"""M9.35 audit. N=10, R=3, sources (4,5,5)/(6,5,5), σ=0.9, α=0.03.

Tries to REFUTE C2b on both-inside.

Writes ../data/m9_35_audit_both.json
"""
...
def main() -> int:
    ...
    n_b = int(both.sum())
    rcb, rfb = rshape(ds[both], pc[both]), rshape(ds[both], pf[both])
    c2b = bool(n_b >= 20 and rcb < rfb)
    payload = {
        "task": "m9.35_audit_both",
        "n": int(len(ds)),
        "n_both": n_b,
        "rho_CHM_both": pearson(ds[both], pc[both]),
        "rho_flat_both": pearson(ds[both], pf[both]),
        "R_CHM_both": rcb,
        "R_flat_both": rfb,
        "std_flat": float(np.std(pf[both])),
        "std_CHM": float(np.std(pc[both])),
        "C2b": c2b,
        "verdicts": {"C2b": "CONFIRMED" if c2b else "REFUTED"},
    }
    os.makedirs(DATA, exist_ok=True)
    path = os.path.join(DATA, "m9_35_audit_both.json")
    with open(path, "w", encoding="utf-8") as fh:
        json.dump(payload, indent=2, fp=fh)
        fh.write("\n")
    print(json.dumps(payload, indent=2))
    return 0 if c2b else 1
