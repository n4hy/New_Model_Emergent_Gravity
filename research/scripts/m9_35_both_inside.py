#!/usr/bin/env python3
"""M9.35: two masses, R=3, both-inside scored with n=69.

Paper 44's Gauss-vs-boost subset had n=11 (R=2). Auditor
refuted C2b. This run uses R=3 so both-inside is 69 balls.

PRE-REGISTERED:
  N=12, R=3, 216 balls. Sources (5,6,6), (7,6,6), σ=1.0.
  Same orthonormal two-source C as M9.34. α=0.02 and 0.05.
  C_vac  |ρ(δS, Tr(K_vac ΔC))| > 0.95
  C0     max|δS| > 1e-6
  C1     Pearson(δS(α), δS(2.5α)) > 0.95
  C2     all balls: R_CHM < R_flat
  C2b    PRIMARY. both-inside (expect n=69): R_CHM < R_flat
  C4     |ρ(δS, P_CHM)| > 0.60 on all balls
  Diagnostic: std(P_flat) vs std(P_CHM) on both-inside.

Not claimed: 8πG, Einstein, de Sitter, MODELS.md.

Writes ../data/m9_35_both_inside.json
"""
...
def main() -> int:
    ...
    pchm, pflat, pk = map(np.asarray, (pchm, pflat, pk))
    both = np.asarray(both, bool)
    n_both = int(both.sum())
    r_chm, r_flat = residual_ratio(ds1, pchm), residual_ratio(ds1, pflat)
    ...
    c2b = bool(n_both >= 20 and r_chm_b < r_flat_b)
    c4g = bool(abs(pearson(ds1, pchm)) > 0.60)
    ok = bool(c_vac and c0g and c1g and c2b)
    if not c_vac:
        verdict = "INSTRUMENT_REJECT"
    elif c2b:
        verdict = "BOTH_INSIDE_CHM"
    else:
        verdict = "BOTH_INSIDE_GAUSS"
    payload = {
        "task": "m9.35_both_inside",
        ...
        "both_inside": {
            "n": n_both,
            "rho_CHM": pearson(ds1[both], pchm[both]),
            "rho_flat": pearson(ds1[both], pflat[both]),
            "R_CHM": r_chm_b,
            "R_flat": r_flat_b,
            "std_P_flat": float(np.std(pflat[both])),
            "std_P_CHM": float(np.std(pchm[both])),
        },
        "C_vac": c_vac,
        "C0_signal": c0g,
        "C1_linear": c1g,
    ...
    path = os.path.join(DATA, "m9_35_both_inside.json")
